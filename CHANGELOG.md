...
* **Campaign Timeline and Pressure Clocks.** Track chapter deadlines, escalating regional threats, and narrative time pressure (including the Stag Lord deadline, troll sightings, Season of Bloom cult events, Varnhold Vanishing rescue timer, and army war pressure). Clocks tick automatically through the turn ticking engine, surface on the kingdom dashboard with pause/resume controls, and emit chat cards and journal entries upon advancing or expiring.
* **Quest and Event Generator.** Automatically convert kingdom random events, rumors, and hex discoveries into actionable, GM-confirmed quests, complications, and rewards linked to the kingdom quest tracker, with distinct GM-only visibility controls and automatic status tracking.
* **Hex Content and Discovery Manager.** Track landmarks, worksites, resources, ruins, refuges, merchants, and enemy armies across the hex grid. Features hidden, discovered, and cleared states synced to Foundry scene drawings, separates player-facing lore from GM-only notes, and suppresses random combat encounters in claimed hexes.
* **Travel and Route Planner.** Calculate overland travel time, route movement costs, and arrival estimates from party speed, mount availability, roads, river crossings, terrain difficulty, weather, and forced march conditions, automatically coordinating with camping watch schedules.
* **Kingdom Turn Assistant (Turn Wizard).** Step-by-step guidance through kingdom upkeep, commerce, leadership, and civic phases. Features a pre-turn checklist, live RP/commodity/consumption previews, configurable leadership activity caps, and an end-turn diff summary before committing changes.
* **Settlement Benefit and Access Tracker.** PC-facing structure benefits viewable directly in Inspect Settlement, mapping structures to class-specific trainers, crafting workshop access, and settlement item purchase levels.
* **Companion Relationships and Personal Quests.** In-character companion profiles linked to actor sheets, tracking companion influence, camp availability, personal quest hooks, and activity gating that dynamically disables absent companion camp activities.
* **Camping Encounter Resolver.** Automated watch encounter engine evaluating watcher Perception vs ambusher Stealth DC, calculating initial enemy distances, sleeping/prone/unconscious states, reaction availability, wake-up checks, and armor comfort traits with an editable GM resolution dialog.
* **Homebrew Rules Profile System.** Switchable rule profiles allowing seamless toggling between RAW Kingmaker, Vance & Kerenshara variants, and custom campaign house rules (XP adjustments, ruin thresholds of 5 vs 10, leadership activity limits, claimed hex combat suppression, and non-capital settlement upgrades).
* **Session Prep and Narrative Recap Dashboard.** GM dashboard aggregating open quests, active deadlines, unresolved kingdom events, companion moments, and nearby hex hooks into an organized session prep outline, paired with an automated narrative prose generator and journal export.
* **Random Encounter and Rumor Curator.** Pacing-focused encounter tables featuring configurable 50/50 combat vs RP encounter weighting, region/level/hex-state filters, and rumor mechanics that unveil map locations or foreshadow looming regional threats.
* **Army and War Pressure Board.** Comprehensive warfare management tracking invading army statuses, march ETAs, and threat clocks, featuring a full 4-phase warfare battle resolver with army XP progression, leveling, casualties, and battle history logging.
* **Balance and Pacing Alerts.** Advisory campaign health monitoring that alerts the GM when kingdom level, unrest, ruin accumulation, magic item access, or claimed worksite ratios drift away from chapter pacing curves.
* **Faction and Diplomacy Relations Tracker.** Living diplomacy system tracking attitude, standing, and diplomatic status for regional factions (Sootscales, Pitax, Brevoy houses, Varnhold, the fey), with standing shifts driven by kingdom actions, treaty/trade agreements, turn drift, and threshold event triggers.
* **Calendar Integration (Seasons & Stars / Simple Calendar).** In-world calendar synchronization logging kingdom turns, camping rests, weather conditions, and companion arrivals; automatically derives seasons from current month and alerts GMs upon month rollover.
* **Campaign Analytics and Trends Dashboard.** Dedicated sheet Analytics tab rendering interactive inline-SVG trend charts for unrest, Resource Points, consumption, fame, XP, war pressure, kingdom level, and all four ruin tracks across configurable history windows.
* **Commodity Market and Caravan Economy.** Establish trade routes with regional partners to dispatch commodity caravans, navigating terrain travel time, route safety, and raid flat checks to deliver bonus Resource Dice and trade revenue.
* **Living Settlement Population and Dynamic Roster.** Settlement population modeling featuring Brevoy and River Kingdoms name generation, automated starter resident rosters, and living settlement life integration.
* **Gear Settings and House Rule Extensions.** Importable and exportable gear settings profiles, Vance & Kerenshara kingdom creation extras (charter/heartland skills and bonus ability boosts), water-adjacency structure rules (Mill consumption), rough terrain construction costs, and anarchy activity gating.
* **Vassal State Annexation and Tribute.** Annex adjacent territories and vassal states, automatically calculating monthly tribute RP accrual, annex unrest penalties, and faction standing adjustments.
